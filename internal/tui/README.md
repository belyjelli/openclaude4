...
- **Slash typeahead:** when the prompt starts with `/` (and the session is not busy), v3-style rows appear above the prompt (slightly inset): **❯** marks the selected line, commands sit in a fixed column with a dim description on the right, and the selection uses a full-width inverted bar (blue background / light text in the default theme)—no separate bordered box. Up to four visible matches plus a dim “+N more” row. **Tab** completes the first token; **Shift+Tab** moves selection backward; **Up** / **Down** move selection when there is more than one match; **Esc** hides the overlay until the input changes. **Shift+Tab** still toggles auto-approve when the overlay is hidden.
- **Toast strip:** a single line under the status block and above the transcript (inset ~2 columns like v3 notifications): errors/warnings use a tinted full-width background; info stays dim. Toasts are **queued** (Charm Crush–style backlog): each message shows ~5s, then the next runs, up to a small max queue so bursts are not dropped silently except when over capacity.
- **Slash stem highlight:** non-selected suggestion rows tint the typed prefix on the command name (same idea as Crush completion **match** styling); the selected row stays a solid inverted bar.
